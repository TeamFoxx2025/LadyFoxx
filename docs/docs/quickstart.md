...
To access the pre-built releases, visit the [GitHub releases page](https://github.com/TeamFoxx2025/LadyFoxx/releases). The client provides cross-compiled AMD64/ARM64 binaries for Darwin and Linux.
...
function initIbftConsensus() {
  ...
  ./foxx-chain secrets init --insecure --data-dir test-chain- --num 4

  node1_id=$(./foxx-chain secrets output --data-dir test-chain-1 | grep Node | head -n 1 | awk -F ' ' '{print $4}')
  node2_id=$(./foxx-chain secrets output --data-dir test-chain-2 | grep Node | head -n 1 | awk -F ' ' '{print $4}')
...
function initPolybftConsensus() {
  ...
  address1=$(./foxx-chain polybft-secrets --insecure --data-dir test-chain-1 | grep Public | head -n 1 | awk -F ' ' '{print $5}')
  address2=$(./foxx-chain polybft-secrets --insecure --data-dir test-chain-2 | grep Public | head -n 1 | awk -F ' ' '{print $5}')
  address3=$(./foxx-chain polybft-secrets --insecure --data-dir test-chain-3 | grep Public | head -n 1 | awk -F ' ' '{print $5}')
  address4=$(./foxx-chain polybft-secrets --insecure --data-dir test-chain-4 | grep Public | head -n 1 | awk -F ' ' '{print $5}')
}

function createGenesis() {
  ./foxx-chain genesis $genesis_params \
    --block-gas-limit 10000000 \
    --premine 0x85da99c8a7c2c95964c8efd687e95e632fc533d6:1000000000000000000000 \
    --premine 0x0000000000000000000000000000000000000000 \
    --epoch-size 10 \
    --reward-wallet 0xDEADBEEF:1000000 \
    --native-token-config "LadyFoxx:LDFX:18:true:$address1" \
...
function initRootchain() {
  ...
  if [ "$1" == "write-logs" ]; then
    ...
    ./foxx-chain rootchain server 2>&1 | tee ./rootchain-server.log &
  else
    ./foxx-chain rootchain server >/dev/null &
  fi
  ...
  ./foxx-chain polybft stake-manager-deploy \
  ...
  while [ $counter -le 4 ]; do
    ...
    ./foxx-chain polybft register-validator \
      --supernet-manager ${customSupernetManagerAddr} \
...
function startServerFromBinary() {
  if [ "$1" == "write-logs" ]; then
    ...
    ./foxx-chain server --data-dir ./test-chain-1 --chain genesis.json \
