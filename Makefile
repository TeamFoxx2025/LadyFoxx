...
build: check-go check-git
	...
	go build -o foxx-chain -ldflags="\
    	-X 'github.com/TeamFoxx2025/LadyFoxx/versioning.Version=$(VERSION)' \
		-X 'github.com/TeamFoxx2025/LadyFoxx/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/TeamFoxx2025/LadyFoxx/versioning.Branch=$(BRANCH)'\
		-X 'github.com/TeamFoxx2025/LadyFoxx/versioning.BuildTime=$(TIME)'" \
	main.go
...
.PHONY: test-e2e
test-e2e: check-go
	go build -race -o artifacts/foxx-chain .
	env EDGE_BINARY=${PWD}/artifacts/foxx-chain go test -v -timeout=30m ./e2e/...

.PHONY: test-e2e-polybft
test-e2e-polybft: check-go
	go build -o artifacts/foxx-chain .
	env EDGE_BINARY=${PWD}/artifacts/foxx-chain E2E_TESTS=true E2E_LOGS=true \
	go test -v -timeout=1h30m ./e2e-polybft/e2e/...

.PHONY: test-property-polybft
test-property-polybft: check-go
	go build -o artifacts/foxx-chain .
	env EDGE_BINARY=${PWD}/artifacts/foxx-chain E2E_TESTS=true E2E_LOGS=true go test -v -timeout=30m ./e2e-polybft/property/... \
	-rapid.checks=10
